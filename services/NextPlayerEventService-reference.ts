...
export const NextPlayerEventService = {
    ...
    const cupDate = new Date(currentDate.getFullYear(), 6, 20).setHours(0, 0, 0, 0);
    if (cupDate >= now) {
      potentialEvents.push({
        startDate: new Date(currentDate.getFullYear(), 6, 20),
        endDate: new Date(currentDate.getFullYear(), 6, 20),
